...
def _single_dot_dep_emitter(out_deps_pairs):
    for outs, deps in out_deps_pairs:
        for out in _pat_whitespacing.split(outs.strip()):
            for dep in _pat_whitespacing.split(deps.strip()):
                if dep:
                    if dep[0] == '#':
                        break
                    yield '\t"%s" -> "%s"\n' % (dep, out)
...
class Test(unittest.TestCase):
    ...
    def test_emitters(self):
        ...
        self.assertEqual(list(_line_emitter('\n')), ['', ''])
        self.assertEqual(list(_line_emitter('a\nb')), ['a', 'b'])
        self.assertEqual(list(_line_emitter('a\\\nb\nc')), ['ab', 'c'])

        self.assertEqual(list(_dependency_emitter(_line_emitter(''))), [])
        self.assertEqual(list(_dependency_emitter(_line_emitter('macro\nout:dep1\\\n dep2\n\tcommand'))), [('out', 'dep1 dep2')])
        self.assertEqual(list(_dependency_emitter(_line_emitter('macro\nout:dep1\\\n dep2\n\tcommand\n\nout2:dep3\\\n dep4\n\tcommand2'))),
                         [('out', 'dep1 dep2'), ('out2', 'dep3 dep4')])

        self.assertEqual(list(_single_dot_dep_emitter(_dependency_emitter(_line_emitter('macro\nout:dep1\\\n dep2\n\tcommand\n\nout2:dep3\\\n dep4\n\tcommand2')))),
                         ['\t"dep1" -> "out"\n', '\t"dep2" -> "out"\n', '\t"dep3" -> "out2"\n', '\t"dep4" -> "out2"\n'])
